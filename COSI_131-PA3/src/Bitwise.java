...
public class Bitwise {
    ...
    public static void set(int i, byte bytes[]) {
    	//get the original bytes to parse, then give the result back to it
    ...
    public static boolean clear(int i, byte bytes[]) {
    	boolean isSet = isset(i%8, bytes[i/8]);
    ...
    public static String toString(byte b) {
    	...
    	int operateByte = 0x80;
    	for (int i = 0; i < 8; i++) {
    		if((operateByte & b) != 0) stringByte += '1';
    		else stringByte += '0';
    		operateByte >>>= 1;
    	}
    	return stringByte;
